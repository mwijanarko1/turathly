import { FileText, BookOpen, Languages, Sparkles, Search, History, GraduationCap } from "lucide-react";
...
const features = [
  {
    ...
    description: "Whether it's a scanned PDF, a handwritten manuscript image, or a modern digital book, Turathly handles it. Our preprocessing engine ensures documents are cleaned and ready for extraction.",
    image: (
      ...
      <div className="relative w-full h-full p-6 flex flex-col gap-4 overflow-hidden group">
        <div className="rounded-xl border border-border bg-background p-4 flex flex-col gap-3 shadow-lg">
          ...
          <div className="font-arabic text-xl leading-relaxed text-right text-foreground">
             ...
             <br />
             إِنَّ مَعَ <span className="bg-accent/10 border-b-2 border-accent/40 px-1 rounded-sm">الْعُسْرِ</span> يُسْرًا
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
             <div className="h-6 px-3 rounded-full bg-muted flex items-center gap-1.5 text-[10px] font-medium border border-border">
               <div className="w-1.5 h-1.5 rounded-full bg-primary" /> Confidence: 99.4%
             </div>
             <div className="h-6 px-3 rounded-full bg-muted flex items-center gap-1.5 text-[10px] font-medium border border-border">
               <div className="w-1.5 h-1.5 rounded-full bg-secondary" /> Script: Naskh
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative w-full h-full p-6 flex flex-col gap-4 overflow-hidden group">
        <div className="rounded-xl border border-border bg-background p-6 shadow-lg">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <p className="text-xs font-bold text-foreground">Scholarly Engine</p>
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Active learning</p>
            </div>
          </div>
          <div className="space-y-4">
             ...
             <div className="p-3 rounded-lg border border-accent/20 bg-accent/5 relative shadow-sm ring-1 ring-accent/10">
               <p className="text-[10px] font-bold text-accent uppercase mb-2">Context-Aware</p>
               <p className="text-sm font-semibold text-foreground font-serif leading-relaxed">"preventing actions that lead to harm"</p>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-32">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto font-light leading-relaxed">
             Beyond translation—a workspace that understands the depth of Islamic literature.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex-1 space-y-8 text-center lg:text-left">
                  <div className={`w-14 h-14 rounded-2xl ${feature.accent} flex items-center justify-center mb-6 shadow-sm mx-auto lg:mx-0 ring-1 ring-border/20`}>
      ...
      <div className="mt-40 border-t border-border bg-muted/10 py-24">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
          <div className="space-y-4">
             ...
             <p className="text-muted-foreground font-light text-sm">Instant access to Hans Wehr and Lisan al-Arab within the workspace.</p>
          </div>
          <div className="space-y-4">
             <div className="w-12 h-12 bg-white rounded-xl shadow-sm border border-border flex items-center justify-center mx-auto mb-6">
                <History className="w-6 h-6 text-secondary" />
             </div>
             <h4 className="font-heading text-xl font-bold">Translation Memory</h4>
             <p className="text-muted-foreground font-light text-sm">Turathly remembers how you translated similar phrases across your projects.</p>
          </div>
          <div className="space-y-4">
             <div className="w-12 h-12 bg-white rounded-xl shadow-sm border border-border flex items-center justify-center mx-auto mb-6">
                <Languages className="w-6 h-6 text-accent" />
             </div>
             <h4 className="font-heading text-xl font-bold">Multi-Lingual</h4>
             <p className="text-muted-foreground font-light text-sm">Full support for English, Indonesian, Urdu, and French target languages.</p>
          </div>
        </div>
      </div>
    </section>
